                    ...
                    <div class="relative grid gap-6 lg:grid-cols-[1fr_340px] lg:items-center">
                        <div>
                            ...
                            <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-300">
                                Esta información permite que el equipo de reclutamiento identifique correctamente tu
                                perfil y
                                pueda contactarte durante el proceso.
                            </p>
                        </div>
                        ...
                        <div class="rounded-3xl bg-white/10 p-5 shadow-xl ring-1 ring-white/10 backdrop-blur">
                            <div class="flex items-center gap-4">
                                <div
                                    class="flex h-20 w-20 flex-shrink-0 items-center justify-center overflow-hidden rounded-3xl bg-white/10 ring-1 ring-white/15">
                                    <img v-if="previewPhoto || candidate?.photo"
                                        :src="previewPhoto || `/storage/${candidate.photo}`" alt="Foto actual"
                                        class="h-full w-full object-cover" />

                                    <span v-else class="material-symbols-outlined text-[38px] text-slate-300">
                                        person
                                    </span>
                                </div>
                                ...
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-black text-white">
                                        {{ candidate?.user?.name ?? 'Candidato' }}
                                    </p>

                                    <p class="mt-1 truncate text-xs font-medium text-slate-300">
                                        {{ candidate?.user?.email ?? 'Sin correo' }}
                                    </p>

                                    <!--<p
                                        class="mt-2 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-slate-200 ring-1 ring-white/10">
                                        Perfil candidato
                                    </p>-->
                                </div>
                            </div>
                ...
                <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 bg-slate-50 px-6 py-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            Los campos de nombre y correo pertenecen a tu usuario y no se editan desde esta sección.
                        </p>
                    </div>
                    ...
                    <form class="p-6" @submit.prevent="submit">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input :value="candidate?.user?.name ?? ''" disabled
                                    class="block w-full cursor-not-allowed rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-slate-500 shadow-sm" />
                            </div>
                            ...
                            <div>
                                ...
                                <input :value="candidate?.user?.email ?? ''" disabled
                                    class="block w-full cursor-not-allowed rounded-2xl border border-slate-200 bg-slate-100 px-4 py-3 text-sm text-slate-500 shadow-sm" />
                            </div>
                                    ...
                                    <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="mt-2 text-xs leading-5 text-slate-400">
                                                Puedes cambiar tu foto de perfil subiendo una nueva imagen.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                            <button type="submit"
                                ...
                                {{ form.processing ? 'Guardando...' : 'Actualizar perfil' }}
                            </button>
                        </div>
                    </form>
                </section>
...
function submit() {
    form.transform((data) => ({
        ...data,
        _method: 'put',
    })).post(route('candidato.profile.update'), {
        forceFormData: true,
    });
}
</script>
